...
module.exports = function (grunt) {
    ...
    grunt.initConfig({
        ...
        clean: {
            ...
            distWin: {
                files: [{
                    dot: true,
                    src: [
                        '<%= config.distWin %>/overcaster.zip',
                        '<%= config.distWin %>/overcaster/*',
                        '<%= config.tmp %>/*'
                    ]
                }]
            }
        ...
        copy: {
            ...
            copyTmpToWin: {
                files: [{
                    expand: true,
                    cwd: '<%= config.tmp %>/',
                    dest: '<%= config.distWin %>/overcaster',
                    src: '**'
                }]
            }
        },
        compress: {
            ...
            finalWindowsApp: {
                options: {
                    archive: '<%= config.distWin %>/overcaster.zip'
                },
                files: [{
                    expand: true,
                    cwd: '<%= config.tmp %>',
        ...
        rename: {
            macApp32: {
                files: [{
                    ...
                    dest: '<%= config.distMac32 %>/overcaster.app'
                }]
            },
            macApp64: {
                files: [{
                    src: '<%= config.distMac64 %>/node-webkit.app',
                    dest: '<%= config.distMac64 %>/overcaster.app'
                }]
            },
            zipToApp: {
                files: [{
    ...
    grunt.registerTask('chmod32', 'Add lost Permissions.', function () {
        var fs = require('fs'),
            path = config.distMac32 + '/overcaster.app/Contents/';
    ...
    grunt.registerTask('chmod64', 'Add lost Permissions.', function () {
        var fs = require('fs'),
            path = config.distMac64 + '/overcaster.app/Contents/';
        fs.chmodSync(path + 'Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
        fs.chmodSync(path + 'Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
        fs.chmodSync(path + 'Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
        fs.chmodSync(path + 'MacOS/node-webkit', '555');
    ...
    grunt.registerTask('createWindowsApp', 'Create windows distribution.', function () {
        ...
        concat([
            'buildTmp/nw.exe',
            'buildTmp/app.nw'
        ], 'buildTmp/overcaster.exe', function () {
            var fs = require('fs');
