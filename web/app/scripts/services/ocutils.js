...
angular.module('overcasterServices')
  .factory('ocUtils', function () {
    ...
    function generateGuid() {
      ...
      return (valueArray[0] + valueArray[1] + "-" + valueArray[2] + "-4" + valueArray[3].substr(0, 3) + "-" + valueArray[4] + "-" + valueArray[5] + valueArray[6] + valueArray[7]).toLowerCase();
    }
    ...
    function getRegexResults(regEx, value) {
      ...
      if (results === undefined || results === null || results.length <= 1) return [];

      else return results.slice(1);
    }
